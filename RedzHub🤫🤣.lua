...
local v466 = vu32:MakeWindow({
    Title = "hieu hub [ REMAKE VERSION BY HIEU ] : Blox Fruits",
...
local vTroll = v466:MakeTab({"Troll", "skull"})
local v487 = v466:MakeTab({"Quest | Items", "swords"})

-- PHẬT TO NHẸ
local BuddhaToggle = false
...
vTroll:AddButton({
   ...
   Callback = function()
      task.spawn(function()
         for i = 1, 10 do -- giữ trong 1 thời gian ngắn
            pcall(function()
...
button.Position = UDim2.new(0.82,0,0.55,0)
button.Text = "↑"
...
button.MouseButton1Click:Connect(function()
    ...
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        root.Velocity = Vector3.new(root.Velocity.X, JumpForce, root.Velocity.Z)
    end
end)

-- TOGGLE NÚT NHẢY
vTroll:AddToggle({
   Name = "Bật/Tắt Nút Nhảy",
   Default = true,
   Callback = function(Value)
      gui.Enabled = Value
   end
...
vTroll:AddSlider({
   ...
   Default = 100,
   Callback = function(Value)
      button.Size = UDim2.new(0, Value, 0, Value)
   end
})

vTroll:AddSection({"Fake Beli"})

local FakeBeliValue = "999999999"
...
vTroll:AddButton({
    Name = "Apply Fake Beli",
    Callback = function()
        pcall(function()
            game.Players.LocalPlayer.Data.Beli.Value = tonumber(FakeBeliValue)
        end)
    end
})
...
v484:AddDiscordInvite({
    Name = "Hieu10772 | Tiktok",
    Description = "Acc Tiktok",
    Logo = "rbxassetid://131723242350068",
    Invite = "https://www.tiktok.com/@hieu10772?_r=1&_t=ZS-955ehAIj1vB"
})
